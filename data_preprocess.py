...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Extract IEMOCAP mel-spectrograms and label mappings"
	)
	parser.add_argument(
		"--root",
		type=Path,
	...
	parser.add_argument("--n-mfcc", type=int, default=40, help="Number of MFCC coefficients")
	parser.add_argument(
		"--feature_type",
		type=str,
		choices=["mel", "mfcc"],
		default="mel",
		help="Acoustic feature type to extract",
	)
	parser.add_argument("--fmin", type=float, default=0.0, help="Minimum frequency")
	parser.add_argument(
		"--fmax",
		type=float,
		default=None,
...
def parse_emotion_labels(iemocap_root: Path) -> dict[str, str]:
	...
	if not eval_files:
		raise FileNotFoundError("No EmoEvaluation .txt files found.")
	...
	if not labels:
		raise RuntimeError("No utterance labels parsed from EmoEvaluation files.")

	if conflicts > 0:
		print(f"[WARN] Found {conflicts} conflicting duplicate label entries; kept first label.")
...
def list_utterance_wavs(iemocap_root: Path) -> list[Path]:
	...
	if not wavs:
		raise FileNotFoundError("No utterance wav files found under Session*/sentences/wav.")
	return wavs
...
def main() -> None:
	...
	with h5py.File(h5_path, "w") as h5f:
		for wav_path in tqdm(wav_files, desc=f"Extracting {args.feature_type} features"):
			key = wav_path.stem
	...
	with csv_path.open("w", newline="", encoding="utf-8") as f:
		writer = csv.DictWriter(
			f,
			fieldnames=["key", "split", "label", "label_id", "num_features", "num_frames", "wav_path"],
		)
		writer.writeheader()
	...
	label_json = {
		...
		"items": [
			{"key": r["key"], "split": r["split"], "label": r["label"], "label_id": r["label_id"]}
			for r in rows
		],
	}
	...
	meta = {
		...
		"top_feature_shapes": [[list(shape), cnt] for shape, cnt in feature_shapes.most_common(10)],
		"files": {
			"h5": str(h5_path),
